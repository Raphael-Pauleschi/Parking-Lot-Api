...
public class ParkingCheckOut {
	...
	public static Double getBill(Parking parking) {
		return getBill(parking.getEntryData(), parking.getExitDate());
	}

	private static Double getBill(LocalDateTime entryData, LocalDateTime exitDate) {
		long minutes = entryData.until(exitDate, ChronoUnit.MINUTES);
		Double bill = 0.0;
